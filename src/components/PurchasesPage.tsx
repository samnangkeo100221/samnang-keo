...
import { Product, PurchaseItem, ExchangeRateConfig } from '../types';
...
interface PurchasesPageProps {
  ...
  onAddPurchase: (purchase: Omit<PurchaseItem, 'id' | 'createdAt'>) => void;
...
export const PurchasesPage: React.FC<PurchasesPageProps> = ({
  ...
  // Form states for active purchase
  ...
  const costUSDNum = parseFloat(unitCostUSD) || 0;
  const costKHRNum = parseFloat(unitCostKHR) || convertUSDtoKHR(costUSDNum, currentRate);
  const totalCostUSD = Number((costUSDNum * qtyNum).toFixed(2));
  const totalCostKHR = Math.round(costKHRNum * qtyNum);

  // Expected profit per unit comparison with product's selling price
  const expectedSalePriceUSD = selectedProduct ? selectedProduct.salePriceUSD : 0;
  const expectedSalePriceKHR = selectedProduct ? selectedProduct.salePriceKHR : 0;
  const profitPerUnitUSD = expectedSalePriceUSD - costUSDNum;
  const profitPerUnitKHR = expectedSalePriceKHR - costKHRNum;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!selectedProduct) {
      setFormError('សូមជ្រើសរើសទំនិញដែលត្រូវទិញចូល');
      return;
    }
    if (qtyNum <= 0) {
      setFormError('ចំនួនទិញចូលត្រូវតែធំជាង ០');
      return;
    }
    if (costKHRNum <= 0 && costUSDNum <= 0) {
      setFormError('តម្លៃទិញជាប្រាក់រៀល ឬដុល្លារត្រូវតែធំជាង ០');
      return;
    }
    ...
    setSuccessMessage(
      `បានកត់ត្រាទិញចូល "${selectedProduct.name}" ចំនួន +${qtyNum} ${selectedProduct.unit} ដោយជោគជ័យ!`
    );

    // Reset invoice and quantity for next purchase
  ...
  const todayPurchases = purchases.filter((p) => p.purchaseDate.startsWith(todayStr));
  const todayTotalUSD = todayPurchases.reduce((sum, p) => sum + p.totalUSD, 0);
  const todayTotalKHR = todayPurchases.reduce((sum, p) => sum + p.totalKHR, 0);
  const todayTotalQty = todayPurchases.reduce((sum, p) => sum + p.quantity, 0);
  ...
  // ទំនិញដែលទិញច្រើនដងជាងគេទាំង៥ (Top 5 Most Frequently Purchased Products)
  const topFrequentPurchases = useMemo(() => {
    ...
    const statsMap = new Map<
      string,
      {
        ...
        imageUrl?: string;
        unit: string;
        purchaseCount: number; // ចំនួនដងដែលបានទិញចូល
        totalQuantity: number; // បរិមាណសរុបដែលបានទិញចូល
        lastCostPriceUSD: number;
        lastCostPriceKHR: number;
        latestPurchaseDate: string;
      }
    >();

    for (const p of purchases) {
      const key = p.productId || p.productCode || p.productName;
      ...
      if (!existing) {
        statsMap.set(key, {
          ...
          lastCostPriceUSD: Number(p.costPriceUSD) || 0,
          lastCostPriceKHR: Number(p.costPriceKHR) || 0,
          latestPurchaseDate: p.purchaseDate || '',
        });
      } else {
        existing.purchaseCount += 1;
        existing.totalQuantity += Number(p.quantity) || 0;
        if (p.purchaseDate && p.purchaseDate >= existing.latestPurchaseDate) {
          existing.latestPurchaseDate = p.purchaseDate;
          existing.lastCostPriceUSD = Number(p.costPriceUSD) || existing.lastCostPriceUSD;
          existing.lastCostPriceKHR = Number(p.costPriceKHR) || existing.lastCostPriceKHR;
        }
      }
    }

    // Sort by purchaseCount descending, then totalQuantity descending
    ...
    return sorted.slice(0, 10).map((item) => {
      const liveProd = products.find(
        (pr) =>
          pr.id === item.productId ||
          (item.productCode && pr.code.toLowerCase() === item.productCode.toLowerCase()) ||
          (item.productName && pr.name.toLowerCase() === item.productName.toLowerCase())
      );
      return {
        ...item,
        productId: liveProd ? liveProd.id : item.productId,
        productName: liveProd ? liveProd.name : item.productName,
        productCode: liveProd ? liveProd.code : item.productCode,
        category: liveProd ? liveProd.category : item.category,
        imageUrl: liveProd?.imageUrl || item.imageUrl,
        unit: liveProd?.unit || item.unit,
        currentStock: liveProd?.stockQuantity,
        minStockAlert: liveProd?.minStockAlert,
        costPriceUSD: liveProd ? liveProd.costPriceUSD : item.lastCostPriceUSD,
        costPriceKHR: liveProd ? liveProd.costPriceKHR : item.lastCostPriceKHR,
      };
    });
  }, [purchases, products]);
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
        <div className="rounded-xl border border-[#2D333E] bg-[#161920] px-3 py-2 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] text-slate-400 font-medium">ទិញចូលថ្ងៃនេះ ($)</span>
          ...
          <span className="text-[10px] text-slate-500 font-mono bg-[#0F1115] px-2 py-0.5 rounded-md border border-[#2D333E]">
            {todayPurchases.length} កំណត់ត្រា
          </span>
        </div>

        <div className="rounded-xl border border-[#2D333E] bg-[#161920] px-3 py-2 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] text-emerald-400 font-medium">ទិញចូលថ្ងៃនេះ (៛)</span>
      ...
      {/* បញ្ជីទំនិញ (យកតែ រូបភាព, ទំនិញ, តម្លៃទិញចូលរៀល) */}
          ...
          <div className="flex items-center justify-between px-3.5 py-2.5 bg-[#12151B] border-b border-[#2D333E]">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-slate-400 hidden sm:inline">
                • ចុចលើជួរទំនិញដើម្បីកត់ត្រាទិញចូល
              </span>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead className="">
                <tr className="bg-[#12151B] border-b border-[#2D333E] text-slate-400 font-semibold shadow-xs">
                  ...
                  {/* ៣. តម្លៃទិញចូលរៀល (buypriceR G:G) */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 items-start">
        ...
        <div className="lg:col-span-7 bg-[#161920] rounded-xl border border-[#2D333E] shadow-sm p-3.5 sm:p-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="rounded-xl border border-blue-500/30 bg-blue-950/20 p-4 space-y-2">
              <div className="flex justify-between items-baseline">
                <span className="text-xs font-bold text-blue-300">
                  សរុបចំណាយទិញចូល ({qtyNum} {selectedProduct?.unit || 'ឯកតា'})៖
                </span>
                ...
                <div className="pt-2 border-t border-blue-500/20 flex justify-between items-center text-[11px]">
                  ...
                  <span
                    className={`font-bold font-mono ${
                      profitPerUnitUSD >= 0 ? 'text-emerald-400' : 'text-rose-400'
                    }`}
                  >
                    {profitPerUnitUSD >= 0 ? '+' : ''}
                    {formatUSD(profitPerUnitUSD)} / ឯកតា ({profitPerUnitKHR.toLocaleString('km-KH')}៛)
                  </span>
                </div>
              )}
            </div>
            ...
            >
              <ArrowDownLeft className="h-4 w-4" />
              <span>កត់ត្រាទិញចូលស្តុក (+ Confirm Stock In)</span>
            </button>
          </form>
        </div>

        {/* RIGHT SIDE: Top 10 Frequent Purchases (ការទិញញឹកញាប់) */}
        <div className="lg:col-span-5 space-y-3">
          <div className="bg-[#161920] rounded-2xl border border-[#2D333E] p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3 pb-2.5 border-b border-[#2D333E]">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    ទំនិញដែលបានទិញច្រើនដងជាងគេ (ចុចដើម្បីជ្រើសរើសទិញ)
                  </p>
                </div>
              </div>
              {onNavigateToReports && (
                <button
                  ...
                  className="text-[11px] text-blue-400 hover:text-blue-300 hover:underline cursor-pointer shrink-0 font-medium"
                >
                  ប្រវត្តិទិញ →
                </button>
              )}
            ...
            {topFrequentPurchases.length === 0 ? (
              <div className="py-10 px-4 text-center text-slate-500 text-xs rounded-xl bg-[#0F1115] border border-dashed border-[#2D333E]">
                ...
                <p className="font-semibold text-slate-400">មិនទាន់មានប្រវត្តិនៃការទិញចូលទេ</p>
                <p className="text-[11px] text-slate-500 mt-1 max-w-xs mx-auto">
                  នៅពេលលោកអ្នកកត់ត្រាទិញទំនិញចូលស្តុក ប្រព័ន្ធនឹងចាត់ចំណាត់ថ្នាក់ទំនិញដែលទិញច្រើនដងជាងគេទាំង ១០ នៅទីនេះ។
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                {topFrequentPurchases.map((item, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative p-2 rounded-xl border transition-all cursor-pointer flex flex-col items-center gap-2 text-center ${
                        isSelected
                          ? 'bg-blue-600/15 border-blue-500 ring-2 ring-blue-500/40 shadow-sm'
                          : 'bg-[#0F1115] border-[#2D333E] hover:bg-[#1A1F29] hover:border-slate-500'
                      }`}
                      title="ចុចដើម្បីជ្រើសរើសទំនិញនេះចូលក្នុងទម្រង់ទិញ"
                    >
                      ...
                      <span className="font-semibold text-white text-[10px] sm:text-[11px] leading-tight line-clamp-2 min-h-[28px] break-words w-full">
                        {item.productName}
                      </span>
                    </div>
                  );
